...
app.get('/admin/tickets/concert/:concertId/settings', async (c) => {
  ...
  // Partituurlijst voor dit concert (gekoppeld via concert_pieces)
  const partituren = await queryAll(c.env.DB, `
    SELECT cp.id as link_id, cp.volgorde, cp.opmerking,
           p.id as piece_id, p.titel as piece_titel, p.toonsoort, p.tempo, p.duur_minuten, p.moeilijkheidsgraad,
  ...
  // Beschikbare stukken (die nog niet in dit concert zitten)
  const availablePieces = await queryAll(c.env.DB, `
    ...
    FROM pieces p
    JOIN works w ON w.id = p.work_id
    WHERE p.id NOT IN (SELECT piece_id FROM concert_pieces WHERE concert_id = ?)
  ...
  return c.html(
    <Layout title={`Instellingen - ${concert.titel}`} user={user}>
      <div class="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* ===================================================== */}
        {/* PARTITUURLIJST — gekoppelde stukken voor dit concert  */}
        {/* ===================================================== */}
        <div class="bg-white rounded-lg shadow-md p-6 mt-8">
          ...
          <p class="text-sm text-gray-500 mb-4">
            Sleep de stukken om de volgorde aan te passen. Leden zien deze lijst op de concert-detailpagina en kunnen direct hun partituur (PDF) downloaden.
          </p>
          ...
          {partituren.length === 0 ? (
            <div class="text-center py-10 text-gray-500 bg-gray-50 rounded-lg border border-dashed">
              ...
              <p>Nog geen stukken gekoppeld aan dit concert.</p>
              <p class="text-xs mt-1">Klik op <strong>"Stuk toevoegen"</strong> om te beginnen.</p>
            </div>
          ) : (
            <ul id="partituren-list" class="space-y-2" data-concert-id={concertId}>
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-gray-600 flex flex-wrap gap-x-3 gap-y-1 mt-0.5">
                      ...
                      <span class={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${matCountMap[p.piece_id] ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
                  ...
                  <button
                    type="button"
                    onclick={`removePartituur(${p.link_id}, ${concertId})`}
          ...
          {/* Hint over partituren beheer */}
          <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800">
            <i class="fas fa-info-circle mr-1"></i>
            <strong>Geen partituren bij een stuk?</strong> Ga naar <a href="/admin/bestanden" target="_blank" class="underline font-semibold">Oefenmateriaal</a> om PDF's per stemgroep te uploaden. De link in dit lijstje verwijst direct naar het juiste stuk.
          </div>
        </div>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          ...
          async function addPartituur(e) {
            ...
            var res = await fetch('/api/admin/concerts/' + fd.get('concert_id') + '/pieces/add', {
              method: 'POST',
            ...
            if (res.ok) {
              ...
              alert('Fout bij toevoegen: ' + (await res.text()));
            }
          }
          window.addPartituur = addPartituur;

          // ── Remove a piece ────────────────────────────────
          async function removePartituur(linkId, concertId) {
            if (!confirm('Dit stuk uit de partituurlijst halen?')) return;
            var res = await fetch('/api/admin/concerts/pieces/' + linkId + '/remove', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            if (res.ok) {
              ...
              alert('Fout bij verwijderen: ' + (await res.text()));
            }
          }
          ...
          async function saveEditPartituur(e) {
            ...
            var res = await fetch('/api/admin/concerts/pieces/' + linkId + '/update', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
            ...
            if (res.ok) {
              ...
              alert('Fout bij opslaan: ' + (await res.text()));
            }
          }
          ...
          (function() {
            ...
            if (!list) return;
            var dragSrc = null;
            var concertId = list.dataset.concertId;
            ...
            list.querySelectorAll('.partituur-item').forEach(function(item) {
              ...
              item.addEventListener('drop', function(e) {
                ...
                if (dragSrc && dragSrc !== item) {
                  ...
                  fetch('/api/admin/concerts/' + concertId + '/pieces/reorder', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
...
// =====================================================
// PARTITUURLIJST API — concert_pieces koppeltabel
// =====================================================

// Voeg een stuk toe aan de partituurlijst van een concert
app.post('/api/admin/concerts/:concertId/pieces/add', async (c) => {
  const concertId = parseInt(c.req.param('concertId'))
  const body = await c.req.json() as { piece_id: number; opmerking?: string }
  const pieceId = parseInt(String(body.piece_id))
  const opmerking = String(body.opmerking || '').trim()

  if (!pieceId) return c.text('piece_id ontbreekt', 400)

  // Bepaal volgende volgorde-nummer
  const max = await queryOne<any>(c.env.DB,
    'SELECT COALESCE(MAX(volgorde), 0) as m FROM concert_pieces WHERE concert_id = ?',
    [concertId])
  const nextOrder = (max?.m || 0) + 10

  try {
    await execute(c.env.DB,
      `INSERT INTO concert_pieces (concert_id, piece_id, volgorde, opmerking)
       VALUES (?, ?, ?, ?)`,
      [concertId, pieceId, nextOrder, opmerking || null])
  } catch (err: any) {
    if (String(err.message || '').includes('UNIQUE')) {
      return c.text('Dit stuk staat al in de partituurlijst', 409)
    }
    throw err
  }
...
// Verwijder een stuk uit de partituurlijst
app.post('/api/admin/concerts/pieces/:linkId/remove', async (c) => {
  const linkId = parseInt(c.req.param('linkId'))
  await execute(c.env.DB, 'DELETE FROM concert_pieces WHERE id = ?', [linkId])
  return c.json({ success: true })
})

// Update opmerking
app.post('/api/admin/concerts/pieces/:linkId/update', async (c) => {
  const linkId = parseInt(c.req.param('linkId'))
  const body = await c.req.json() as { opmerking?: string }
  const opmerking = String(body.opmerking || '').trim()

  await execute(c.env.DB,
    'UPDATE concert_pieces SET opmerking = ? WHERE id = ?',
    [opmerking || null, linkId])

  return c.json({ success: true })
})

// Herorder de partituurlijst (drag & drop)
app.post('/api/admin/concerts/:concertId/pieces/reorder', async (c) => {
  const concertId = parseInt(c.req.param('concertId'))
  const body = await c.req.json() as { link_ids: number[] }
  const linkIds = (body.link_ids || []).map((x: any) => parseInt(x)).filter((x: number) => !isNaN(x))

  if (linkIds.length === 0) return c.text('Geen IDs', 400)

  // Update volgorde in stappen van 10 (zodat handmatige tussenvoegingen later mogelijk blijven)
  const stmts = linkIds.map((id: number, idx: number) =>
    c.env.DB.prepare('UPDATE concert_pieces SET volgorde = ? WHERE id = ? AND concert_id = ?')
      .bind((idx + 1) * 10, id, concertId)
  )
